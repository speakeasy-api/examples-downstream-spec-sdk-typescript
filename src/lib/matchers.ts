...
import { ExampleAPIDefaultError } from "../models/errors/exampleapidefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | ExampleAPIDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | ExampleAPIDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ExampleAPIDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ExampleAPIDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
